...
// Accepts CSV or tab/comma separated lines with columns: name, email, phone?, goal?
...
interface ParsedRow { full_name: string; email: string; phone: string; goal: string; valid: boolean; reason?: string }
...
function parse(text: string, existingEmails: Set<string>): ParsedRow[] {
  ...
  return lines.slice(start).map((line) => {
    ...
    return { full_name, email, phone, goal, valid, reason };
  });
}
                ...
                <div className="rounded-[10px] overflow-hidden" style={{ border: '1px solid var(--border)' }}>
                  ...
                  <div className="overflow-x-auto">
                    <div style={{ minWidth: 520 }}>
                      <div className="grid px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.5px]" style={{ gridTemplateColumns: '1.4fr 1.8fr 1.2fr 1fr 0.9fr', background: 'var(--bg2)', color: 'var(--text3)', borderBottom: '1px solid var(--border)' }}>
                        <div>Name</div><div>Email</div><div>Phone</div><div>Goal</div><div>Status</div>
                      </div>
                      <div style={{ maxHeight: 220, overflowY: 'auto' }}>
                        {rows.map((r, i) => (
                          <div key={i} className="grid items-center px-3 py-1.5" style={{ gridTemplateColumns: '1.4fr 1.8fr 1.2fr 1fr 0.9fr', borderBottom: '1px solid var(--border)', opacity: r.valid ? 1 : 0.55 }}>
